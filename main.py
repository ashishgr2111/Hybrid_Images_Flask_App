...
def channel(folder):					
	image = cv2.imread(folder + 'image1.jpeg')     
	image = cv2.resize(image, (200,200), interpolation = cv2.INTER_CUBIC)
	image2 = cv2.imread(folder + 'image2.jpeg')
	...
	# cv2.waitKey(0)
	# cv2.destroyAllWindows()
	cv2.imwrite(folder + 'hybrid.jpeg', np.int32(full*255))
	#--------------------------------------------------------------------------------
	...
	# cv2.imwrite(folder + 'GaussianPyr.jpeg', np.int32(images*255))
	# #--------------------------------------------------------------------
	...
	# cv2.imwrite(folder + 'LaplacianPyr.jpeg', np.int32(images*255))
	# #--------------------------------------------------------------------
	...
	# corrMatrix = df.corr()
	# print (corrMatrix)
	# fig = sn.heatmap(corrMatrix, annot=True, cmap="gray").get_figure()
	# fig.show()
	# fig.savefig(folder + 'CorrelationMatrix.png')
